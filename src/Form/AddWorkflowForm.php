...
class AddWorkflowForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['workflow_image_upload_wrapper']['workflow_image_upload'] = [
      ...
      '#default_value' => $form_state->getValue('workflow_image_upload') ?: NULL,
      '#upload_validators' => [
    ...
    if ($image_type === 'url') {
      $image_view_url = (string) $form_state->getValue('workflow_image_url');
    }
    ...
    $form['workflow_webdocument_upload_wrapper']['workflow_webdocument_upload'] = [
      ...
      '#default_value' => $form_state->getValue('workflow_webdocument_upload') ?: NULL,
      '#upload_validators' => [
    ...
    $webdoc_view_url = '';
    $webdoc_type = $form_state->getValue('workflow_webdocument_type');
    if ($webdoc_type === 'url') {
      $webdoc_view_url = (string) $form_state->getValue('workflow_webdocument_url');
    }
    ...
    if ($webdoc_view_url !== '') {
      $form['workflow_webdocument_preview'] = [
        '#type' => 'markup',
        '#markup' => Markup::create(
          '<div class="mt-2">'
